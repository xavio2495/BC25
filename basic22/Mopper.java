...
public class Mopper extends Unit {
    ...
    Mopper(RobotController rc) throws GameActionException {
        super(rc);
        useComms = Math.random() < Constants.COMMS_USE_PROBABILITY_MOPPER;
    }
    ...
    MapLocation getTarget() throws GameActionException {
        if (recovering && TowerManager.closestPaintTower != null) {
            rc.setIndicatorString("recovering " + TowerManager.closestPaintTower);
            rc.setIndicatorDot(TowerManager.closestPaintTower, 255, 0, 255);
            return TowerManager.closestPaintTower;
        }
        MapLocation target = getClosestEnemy();
        if(target != null) {
            rc.setIndicatorString("enemy " + target);
            rc.setIndicatorDot(target, 255, 0, 255);
            return target;
        }
        
        target = getClosestEnemyPaint();
        if(target != null) {
            rc.setIndicatorString("paint " + target);
            rc.setIndicatorDot(target, 255, 0, 255);
            return target;
        }

        if (2*rc.getPaint() > rc.getType().paintCapacity + Constants.MIN_GIVING_THRESHOLD) {
            target = searchClosestHurt();
            if(target != null) {
                rc.setIndicatorString("hurt " + target);
                rc.setIndicatorDot(target, 255, 0, 255);
                return target;
            }
        }
        ...
        if(useComms) {
            MapLocation loc = Comms.getTarget();
