...
public class Soldier extends Unit {
    ...
    void paintNearby() throws GameActionException {
        ...
        ResourcePatternManager.paintNearby();

        if (rc.senseMapInfo(rc.getLocation()).getPaint() == PaintType.EMPTY){
            if (rc.canAttack(rc.getLocation()) && !Map.isNearRuin(rc.getLocation())){
    ...
    static boolean tryPaint(MapLocation loc, PaintType targetPaint) throws GameActionException {
        if (!rc.canSenseLocation(loc)) return false;
        MapInfo m = rc.senseMapInfo(loc);
        //if (m.isWall() || m.hasRuin()) return false;
        if (!m.isPassable() || (!Util.towerMax() && Map.isNearRuin(m.getMapLocation()))) return false;
    ...
    MapLocation getTarget() throws GameActionException{
        ...
        if (tg != null){
            ...
            //if (tg != null) rc.setIndicatorDot(tg, 0, 200, 0);
