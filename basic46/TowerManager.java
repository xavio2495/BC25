...
public class TowerManager {
    ...
    static int getNextBuild() {
        ...
        if (MyRobot.rc.getRoundNum() < Constants.PAINT_TOWER_LATE_RD){
            if (ch < Constants.MIN_CHIPS_PAINT_TOWER) return MONEY;
            return PAINT;
        }
        if (ch < Constants.MIN_CHIPS_PAINT_TOWER_LATE) return MONEY;
        return PAINT;
    ...
    static boolean shouldDestruct() throws GameActionException {
        for (Direction dir : MyRobot.directions){
            if (MyRobot.rc.senseMapInfo(MyRobot.rc.getLocation().add(dir)).getMark() != PaintType.EMPTY) return false;
        }
