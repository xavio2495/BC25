...
public class Tower extends MyRobot {
    ...
    static final int MID_GAME_ROUND = 250;

    static final int LATE_GAME_ROUND = 1700;

    int roundMopper = -ROUNDS_PROTECTION;

    static boolean mopperSpawned = false;
    ...
    Tower(RobotController rc) throws GameActionException {
        ...
        if (creationTurn < 5) spawnPlan = spawnPlanInitial;
        else spawnPlan = spawnPlanEarly;
    ...
    UnitType[] wantedSpawnPlan() {
        ...
        if(round < MID_GAME_ROUND) return spawnPlanEarly;
        if(round < LATE_GAME_ROUND) return spawnPlanMid;
    ...
    void runTurn() throws GameActionException {
        ...
        m = getEnemyPaint();
        if (m != null && shouldSpawn(UnitType.MOPPER)) spawnDefense(m);
    ...
    void spawnDefense(MapLocation target) throws GameActionException {
        ...
        if (bestLoc != null) CustomBuild(UnitType.MOPPER, bestLoc);
    }
    ...
    void spawnRush() throws GameActionException {
        ...
        if (bestLoc != null){
            CustomBuild(UnitType.SOLDIER, bestLoc);
    ...
    void spawn(UnitType t) throws GameActionException {
        ...
        if (rc.canBuildRobot(t, rc.getLocation().add(dir).add(dir))){
            CustomBuild(t, rc.getLocation().add(dir).add(dir));
        ...
        if (rc.canBuildRobot(t, rc.getLocation().add(dir))){
            CustomBuild(t, rc.getLocation().add(dir));
            dirCount[dir.ordinal()]++;
            increaseSpawnPlan();
        }
    }

    void increaseSpawnPlan() {
        spawnPlanPos++;
    ...
    static void CustomBuild(UnitType t, MapLocation m) throws GameActionException {
        rc.buildRobot(t, m);
        if (t == UnitType.MOPPER) mopperSpawned = true;
    }

    static MapLocation getEnemyPaint() throws GameActionException {
        MapLocation ans = getClosestEnemyPaint();
        if (!mopperSpawned && alliedMoppers == 0) return ans;
        return null;
    }
