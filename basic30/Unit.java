    ...
    void tryWithdraw() throws GameActionException {
        ...
        for (RobotInfo r : robots){
            if (r.getType().isTowerType()){
                ...
                if (neededPaint > theirPaint) neededPaint = theirPaint;
                int afterPaint = theirPaint - neededPaint;
                if (afterPaint <= remainingPaint && afterPaint%100 > 100 - Constants.PAINT_SAVINGS){
                    neededPaint -= (100 - afterPaint%100);
                    if (neededPaint <= 0) return;
                }
                /*switch(r.getType()){
