...
public class Tower extends MyRobot {
    ...
    static final int ROUNDS_PROTECTION = 40;

    static int roundMopper = -ROUNDS_PROTECTION;

    static int enemySoldiersSeen;
    static int alliedMoppersSeen;

    static int[] soldierDirCount = new int[8];
    static int[] mopperDirCount = new int[8];
    static int[] splasherDirCount = new int[8];
    double[] dists = new double[8];

    static final UnitType[] spawnPlanInitial = {
            UnitType.SOLDIER,
            UnitType.SOLDIER,
            UnitType.SOLDIER,
            UnitType.SOLDIER,
    ...
    void runTurn() throws GameActionException {
        ...
        if(m != null) spawnDefense(m);


        UnitType t = switch (rc.getType()){
            case LEVEL_ONE_PAINT_TOWER, LEVEL_TWO_PAINT_TOWER, LEVEL_THREE_PAINT_TOWER -> getNextSpawn();
            default -> getNextSpawnMoney();
        };
    ...
    UnitType getNextSpawnMoney(){
        if (rc.getPaint() >= UnitType.SPLASHER.paintCost && rc.getNumberTowers() >= Constants.MIN_TOWERS_SPLASHER) return UnitType.SPLASHER;
        if (rc.getPaint() <= UnitType.MOPPER.paintCost) return UnitType.MOPPER;
    ...
    static MapLocation getThreat() throws GameActionException {
        ...
        RobotInfo[] units = rc.senseNearbyRobots(GameConstants.VISION_RADIUS_SQUARED);
        MapLocation bestAns = null;
        int bestDist = 0;
        for (RobotInfo r : units){
            switch(r.getType()){
                case SOLDIER:
                    if (r.getTeam() == rc.getTeam()) break;
                    if (!BFSTower.isClose(r.getLocation())) break;
                    ++enemySoldiersSeen;
        ...
        //if (alliedMoppersSeen > enemySoldiersSeen + 1) return null;
        return bestAns;
    }
